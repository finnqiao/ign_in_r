...
```{r}
ign %>%
  subset(., release_year>1990 & release_year<2000) %>%
  ...
  # geom_text(aes(x=release_year, y=total_games, label=platform),check_overlap=TRUE) +
  geom_dl(aes(x=release_year, y=total_games, label = platform), method = list(dl.combine("last.points"), cex = 0.8)) +
  guides(col=FALSE)
